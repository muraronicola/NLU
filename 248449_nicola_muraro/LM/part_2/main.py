...
if __name__ == "__main__":
    ...
    if not eval_only: #Train the models and evaluate all of them
        ...
        optimizer = optim.SGD(third_model.parameters(), lr=5)
        scheduler = lr_scheduler.LinearLR(optimizer, start_factor=0.9, end_factor=1, total_iters=20)
        ...
        best_model = LM_LSTM(emb_size=600, hidden_size=600, output_size=len(lang.word2id), emb_dropout=0.5, out_dropout=0.5, pad_index=lang.word2id["<pad>"], device=device).to(device)
