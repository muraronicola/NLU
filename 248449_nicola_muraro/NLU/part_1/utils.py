...
class LoadData():
    ...
    def __init__(self, train_path, test_path, device="cpu", pad_token=0, lang=None):
        ...
        self.__test_dataset = IntentsAndSlots(self.__test_raw, self.__lang)
    ...
    def __createDatasetInformation(self, train_raw, dev_raw, dev_test):
        words = sum([x['utterance'].split() for x in train_raw], []) # No set() since we want to compute 
                                                            # the cutoff
        corpus = train_raw + dev_raw + dev_test # We do not wat unk labels, 
                                                # however this depends on the research purpose
        slots = set(sum([line['slots'].split() for line in corpus],[]))
        intents = set([line['intent'] for line in corpus])
        
        return words, corpus, intents, slots
